...
from app.models.appointment import Appointment,CreateAppointment
...
async def create_appointment(appointment: CreateAppointment):
...
async def update_appointment(appointment_id: str, appointment: CreateAppointment):
    appointment.updatedAt = datetime.utcnow()
    appointment_dict = appointment.dict(by_alias=True, exclude_unset=True)
    result = await db["appointments"].update_one({"_id": ObjectId(appointment_id)}, {"$set": appointment_dict})
