...
async def create_lab(lab: Lab):
    lab_dict = lab.dict(by_alias=True)
    result = db["labs"].insert_one(lab_dict)
...
async def update_lab(lab_id: str, lab: Lab):
    lab_dict = lab.dict(by_alias=True, exclude_unset=True)
    result = db["labs"].update_one({"_id": ObjectId(lab_id)}, {"$set": lab_dict})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Lab not found")
    updated_lab = db["labs"].find_one({"_id": ObjectId(lab_id)})
    return Lab(**updated_lab)

@lab_router.delete("/{lab_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_lab(lab_id: str):
    result = db["labs"].delete_one({"_id": ObjectId(lab_id)})
