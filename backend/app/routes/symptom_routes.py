...
async def create_symptom_analysis(symptom_analysis: SymptomAnalysis):
    ...
    result = db["symptom_analysis"].insert_one(symptom_analysis_dict)
...
async def get_symptom_analysis(chat_id: str):
    symptom_analysis = db["symptom_analysis"].find_one({"_id": ObjectId(chat_id)})
...
async def update_symptom_analysis(chat_id: str, symptom_analysis: SymptomAnalysis):
    symptom_analysis.timestamp = datetime.utcnow()
    symptom_analysis_dict = symptom_analysis.dict(by_alias=True, exclude_unset=True)
    result = db["symptom_analysis"].update_one({"_id": ObjectId(chat_id)}, {"$set": symptom_analysis_dict})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Symptom analysis not found")
    updated_symptom_analysis = db["symptom_analysis"].find_one({"_id": ObjectId(chat_id)})
...
async def delete_symptom_analysis(chat_id: str):
    result = db["symptom_analysis"].delete_one({"_id": ObjectId(chat_id)})
